...
def run_experiment(method, model_instance, *args, **kwargs):
    runner = ExperimentRunner()
    if method == 'supervised':
        return runner.run_supervised_baseline(model_instance, *args, **kwargs)
    elif method == 'rule_space':
        return runner.run_rule_space_ssl(*args, **kwargs)
    elif method == 'mf_space':
        return runner.run_mf_space_ssl(*args, **kwargs)
    elif method == 'raw_space':
        return runner.run_raw_space_ssl(*args, **kwargs)
    elif method == 'fmv_clp':
        return runner.run_fmv_clp(*args, **kwargs)
    elif method == 'mv_grf':
        return runner.run_mv_grf(*args, **kwargs)
    else:
